...
const db = knex({
    client: "pg",
    connection: "postgresql://postgres:mNKvxWiVbOnCZUzAZMAWMWlBJfLehgXB@autorack.proxy.rlwy.net:23862/railway", // String koneksi PostgreSQL
  });
  ...
  (async () => {
    try {
      ...
      db.destroy(); // Tutup koneksi setelah selesai
    } catch (err) {
      console.error("Database connection failed:", err.message); // Tangani error koneksi
      db.destroy(); // Pastikan koneksi ditutup jika ada error
    }
  })();
